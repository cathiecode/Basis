...
use crate::{extract, logging, logging::UnityDlpLogCallback, python_host};

// ── Logging ──────────────────────────────────────────────────────────────────

/// Register the callback used for native log messages.
///
/// The callback may run on any thread and remains registered until
/// `unity_dlp_clear_log_callback` is called.
#[no_mangle]
pub extern "C" fn unity_dlp_set_log_callback(callback: UnityDlpLogCallback) {
    logging::init();
    logging::set_callback(Some(callback));
}

/// Clear the native log callback, waiting for any callback already in progress
/// to return.
#[no_mangle]
pub extern "C" fn unity_dlp_clear_log_callback() {
    logging::set_callback(None);
}
