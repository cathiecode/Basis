...
# _extract(_url, _opts, _result_object)
...
if not extract_thread.is_alive():
    # Extract thread successfully finished or raised exception
    _result = _result_object['result']
else:
    # Extract thread didn't finished or raised exception in time

    # Get a thread id for extract thread
    ...
    # Raise exception in extract thread forcibly
    ...
    # Raise exception for this thread
